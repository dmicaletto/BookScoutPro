...
import PublicInfoDetailModal from './PublicInfoDetailModal'

const PromotionBanner = () => {
  const { info, loading }           = usePublicInfo()
  const [selected, setSelected]     = useState<PublicInfo | null>(null)
    ...
    <>
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {info.map((item) => {
            const coverPhoto = item.photos?.[0] ?? item.image
            return (
              <div
                key={item.id}
              ...
              >
                {coverPhoto && (
                  <div className="absolute top-0 right-0 w-32 h-32 opacity-10 -mr-8 -mt-8 rotate-12 group-hover:rotate-6 transition-transform">
                    <img src={coverPhoto} alt="" className="w-full h-full object-contain" />
                  </div>
                )}
                ...
                <div className="flex items-start justify-between mb-4">
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                    ...
                    {item.type === 'promo' ? 'Promo' : item.type === 'event' ? 'Evento' : 'Info'}
                  </span>
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-emerald-600 text-xs font-bold hover:gap-2 transition-all uppercase"
                >
